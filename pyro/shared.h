...
# include "rc.h"
//# include <typeinfo>
template<class T> struct __shared__ final
{
...
//		printf(" shared<%s> dtor n=%u\n", typeid(T).name( ), n);
	...
	__shared__<T> & operator = (__shared__<T> && a)noexcept
	{
...
# pragma warning(suppress: 6308)
	...
	{
		for(size_t i(0UL); (i < n); ++i) if( U[i] == ULL ) // o(n)
		{
			A[i].~T( ); // dtor
			if( n > 1UL )
			{
...
namespace pyro
{
	template<class T> static inline __shared__<T> shared;
}
